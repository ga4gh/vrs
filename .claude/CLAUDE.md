...
- **Closed subtype unions:** the abstract classes (`Variation`, `MolecularVariation`, `SystemicVariation`, `Location`, `SequenceExpression`) carry a class-level `oneOf` enumerating their concrete subtypes plus a `discriminator: {propertyName: type}`, so a value referenced at the abstract level must validate as one of those subtypes. Concrete classes pin their `type` `const` and stay closed. (The other `oneOf`s in the source are *use-site* property refs like `location: oneOf: [iriReference, SequenceLocation]`, not class-level unions.)
